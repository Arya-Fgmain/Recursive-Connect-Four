...
public class Sys {
  ...
  public final static int SLOT__FULL        = -2;
  ...
  /* Recursively checks the grid for diagonal matches (up-right direction) in 4-slot groups
   ...
   */
  public boolean rightDigCheck(int startR, int startC) {
    ...
    return this.rightDigCheck(startR - 1, startC + 1); // Recursive call to continue the check for later diagonals (or later part of a diagonal)
  }
  
  /* Recursively checks the grid for diagonal matches (down-right direction) in 4-slot groups
   * @param startR - The starting row to check from
   * @param c      - The starting column to check from
   * @return       - Returns true if a match has been made,
   *               - Returns false if no matches have been made
   */
  public boolean leftDigCheck(int startR, int startC) {
  ...
  /* Recursively checks the grid for row matches in 4-slot groups
   * @param R           - The row from which to check
   * @param startc      - The column to check from
   * @return            - Returns true if a match has been made,
   *                    - Returns false if no matches have been made
   */
  public boolean rowCheck(int R, int startC) {
    ...
    return this.rowCheck(R, startC + 1); // Recursive call to continue searching the column
  }
  
  public boolean colCheck(int startR, int C) {
    
    if (startR + 3 >= GRID_ROW) {
      return false;
    }
    ...
    return this.colCheck(startR + 1, C);
    
  }
  
  public boolean CheckRightDig(int startR, int startC) { // Upright second half ***
    ...
    if ( startR - 3 < 0 || startC + 3 >= GRID_COL ) {
  ...
  public boolean CheckLeftDig(int startR, int startC) { // Upright second half
  ...
  public boolean CheckRow(int startR) {
  ...
  public boolean CheckCol(int startC) {
