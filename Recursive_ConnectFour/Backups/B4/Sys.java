public class Sys {
  ...
  public Sys() {
    ...
    //this.grid[1][6] = new Slot(Slot.VAL_BLK);
  }
  ...
  public void insertSlot(int startR, int C, Slot newSlot) {
    
    if (!( (startR >= this.grid.length || startR < 0) || ( C >= this.grid[0].length || C < 0 ) )) {
      if (!this.isSlotFull(startR, C)) {
        if (startR + 1 < this.grid.length && !this.isSlotFull(startR+1, C)) {
          this.insertSlot(startR+1, C, newSlot);
        } else {
